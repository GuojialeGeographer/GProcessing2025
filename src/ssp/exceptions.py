"""
Custom exceptions for SpatialSamplingPro package.

This module defines all custom exceptions used throughout the SpatialSamplingPro package,
...
class SpatialSamplingProError(Exception):
    """
    Base exception class for all SpatialSamplingPro errors.

    All custom exceptions in SpatialSamplingPro inherit from this base class,
    allowing users to catch all SpatialSamplingPro-specific errors with a single
    except clause.
    ...
    Example:
        ...
        ...     print(f"SpatialSamplingPro error: {e}")
    """

    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None):
        """
        Initialize SpatialSamplingPro error.
...
class ConfigurationError(SpatialSamplingProError):
    """
...
class BoundaryError(SpatialSamplingProError):
    """
...
class SamplingError(SpatialSamplingProError):
    """
...
class NetworkDownloadError(SpatialSamplingProError):
    """
...
class ValidationError(SpatialSamplingProError):
    """
...
class ExportError(SpatialSamplingProError):
    """
...
class VisualizationError(SpatialSamplingProError):
    """
...
def format_error_context(error: Exception, include_traceback: bool = False) -> str:
    ...
    if isinstance(error, SpatialSamplingProError) and error.details:
        lines.append("\n📋 Details:")
